...
F = F_Eu + F_Al + F_Ga #+ 0.1*np.random.rand(len(k2d), len(k2d)) + 1.j * np.random.uniform(len(k2d), len(l2d))
"""Intensity I"""
...
for i in range(0, 2*kmax + 1): # for ∆k=0.1, up to 2*kmax=lmax + 1
...
if deltak == 0.1:
    for i in range(0, 2*kmax + 1):
        l_intlist = np.delete(l_intlist, i*9)  # Lösche jeden zehnten index, 9 for ∆k=0.1
    for i in l_intlist: # Set unallowed L-values for intensities to 0
        I[i,:] = 0
else:
    for i in range(0, 2*kmax*10 + 1):
        l_intlist = np.delete(l_intlist, i*99)  # Lösche jeden zehnten index
    for i in l_intlist: # Set unallowed L-values for intensities to 0
        I[i,:] = 0
##############################################################################
# Noise
noisefactor = 0.2 # Amplitude of the noise for the intensity
Igaussian = np.ones((len(k2d), len(l2d)))
for i in len(k2d):
    IGaussian = IGaussian + np.exp(-((k2d-np.ones((len(k2d), len(l2d)))*2)**2+l2d**2))
I = I + noisefactor*np.random.rand(len(k2d), len(k2d)) + np.exp(-((k2d-np.ones((len(k2d), len(l2d)))*2)**2+l2d**2)) # Add random noise with maximum 1
# Plotting
...
# plt.contourf(l2d, k2d, I, cmap='viridis', extent=(k0-kmax, k0+kmax, l0-lmax, l0+lmax), origin='lower',
