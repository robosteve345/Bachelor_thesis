...
def main():
    ...
    # Strcture parameters
    a = 4.34076  # in angstrom
    c = 10.93791 # in angstrom  experimentally for T = 293 K
    z0 = 0.3869  # Equilibrium 4e Wyckoff z parameter.
    ...
    B_iso_Eu, B_iso_Ga, B_iso_Al = 0, 0, 0 # isotropic movements around equilibrium, inverse proportional to the mass of the atom
    ...
    for i in range(2):  # 2 atoms for each sort, for Eu with 1 position and 2 for each al and ga with translation
        ...
        F_Al1_list.append(
            f_Al * np.exp(-B_iso_list[2] / lamb * (np.sin(theta))**2) * np.exp(1j * 2 * np.pi * (h * np.ones((n, n)) *Al1[0, i] + k2d *Al1[1, i] + l2d *Al1[2, i]))
            )
        F_Al1_T_list.append(
            f_Al * np.exp(-B_iso_list[2] / lamb * (np.sin(theta))**2) * np.exp(1j * 2 * np.pi * (h * np.ones((n, n)) *Al1_T[0, i] + k2d *Al1_T[1, i] + l2d *Al1_T[2, i]))
            )
        F_Al2_list.append(
           f_Al * np.exp(-B_iso_list[2] / lamb * (np.sin(theta))**2) * np.exp(1j * 2 * np.pi * (h * np.ones((n, n)) *Al2[0, i] + k2d *Al2[1, i] + l2d *Al2[2, i]))
           )
        F_Al2_T_list.append(
           f_Al * np.exp(-B_iso_list[2] / lamb * (np.sin(theta))**2) * np.exp(1j * 2 * np.pi * (h * np.ones((n, n)) *Al2_T[0, i] + k2d *Al2_T[1, i] + l2d *Al2_T[2, i]))
           )
