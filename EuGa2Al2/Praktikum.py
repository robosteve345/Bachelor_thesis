...
def lin_reg(x, y, dy, sigma_y, Titel=None, dx=None, sigma_x=None, plot=False, p=None,
            xaxis=None, yaxis=None):
    """Berechnet linearen fit zu gegebenen 2D-Datensatz in x und y. Zeichnet
    Fehlerschläuche, Hilfsgeraden und berechnet syst. und stat. MUs der
    Parameter m und t der fit-Funktion:          y = m*x + t
    --------------
    x: Abszissenarray
    y: Ordinatenarray
    dx: syst. MU der Abszissenwerte. Default=None
    ...
    # print("X(90K)={}".format(lin_fit(90, *par2)))
    ...
    if plot == True:
        ...
        # Experimentelle Daten mit Fehlerbalken
        plt.plot(x, y, marker='o', c='k', ls='', ms=5#, label='experimental data'
                 )
        #plt.errorbar(x, y, yerr=sigma_y, c='k',
        #             ls='', capsize=3#, label='stat. MU'
        #             )
        ...
        plt.plot(x=np.array([5.06]),
                     y=np.array([250]),
                     c='tab:blue', marker='o', label='Experimental Data', capsize=5, ls='')
        # plt.savefig("{}.png".format(Titel), dpi=300)
...
def linreg(x, y, ax=None, dx=None, dy=None, dysys=None, dxsys=None, plot=True):
    ...
    """
    # fit
    popt, pcov = fit(linear, x, y, sigma = dy,
                        p0 = [0,0], absolute_sigma=True) 

    # Systematische Unsicherheiten
    if np.all(dxsys != None):
        if np.all(dysys != None):
            covh, v = fit(linear, x + np.sign(popt[0])*dxsys,
                                y - dysys, sigma=dy,
                                  p0 = [0,0], absolute_sigma=True)
            covl, v = fit(linear, x - np.sign(popt[0])*dxsys,
                                y + dysys, sigma=dy,
                                  p0 = [0,0], absolute_sigma=True)
        else:
            covh, v = fit(linear, x - np.sign(popt[0])*dxsys, y, sigma=dy,
                                  p0 = [0,0], absolute_sigma=True)
            covl, v = fit(linear, x+np.sign(popt[0])*dxsys, y, sigma=dy,
                                  p0 = [0,0], absolute_sigma=True)   
